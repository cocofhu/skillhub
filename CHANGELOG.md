...
- Failed to load plugins 时提供粗暴快速修复：Host 在失败页注入「快速修复 · 卸载全部第三方」，保留基线并清空第三方，CLI 为 `skillhub-recovery nuke-third-party`。健康 boot 的 GET `/skillhub/recovery.js` 不会武装 nonce。说明见 [docs/recovery.md](docs/recovery.md)
